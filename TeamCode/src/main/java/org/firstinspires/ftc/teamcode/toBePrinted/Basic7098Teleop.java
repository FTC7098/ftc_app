...
public class Basic7098Teleop extends OpMode
{
	...
	public void loop()
	{
		...
		// Scales the movement of the robot proportional to the cube of the amount the joystick is moved. This makes it so more precision can be gained when moving at slow speeds.
		...
		// If the touch sensor is enabled, and it is not being pressed, that will cause it to load the shooter, but not fire, since fully loading will press the sensor
		...
		// A and B buttons on the second controller are used to move the servo that holds up the forklift.
		if (gamepad2.a)
		{
		...
		{
			// Since getting the servo value initially returns NaN, we must set a value the first time we use it
		...
		// Driver #2 can use their D-PAD to raise and lift our forklift (Note that there are two motors, part of our speed-based design)
		if (gamepad2.dpad_up)
		{
			robot.moveMotor(6, 1);
			robot.moveMotor(7, 1);
